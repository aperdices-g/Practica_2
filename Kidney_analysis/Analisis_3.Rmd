...
```{r}
datos<-read.csv("data/kidney_disease_clean_3.csv",sep=",",header = TRUE)
...
Como podemos observar en todos los campos tenemos valores extremos. Como se ha indicado anteriormente, no prescindiremos de ellos ya que pueden aportar información al estudio.(Se han eliminado en el proceso de limpieza aquellos valores no compatibles)
...
```{r}
par(mfrow=c(1,2))
boxplot(datos$hemo~datos$htn, main="hemo~htn")
boxplot(datos$rc~datos$htn, main="rc~htn")
```
...
```{r}
par(mfrow=c(1,2))
boxplot(datos$sc~datos$htn, main="sc~htn")
boxplot(datos$rc~datos$htn, main="rc~htn")
```
